            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <label class="form-label">{{ $t('global.latitude') }}</label>
                            <input type="number" class="form-control" v-model="v$.latitude.$model"
                                :placeholder="$t('global.latitude')" :class="{
                                    'is-invalid': v$.latitude.$error || errors[`latitude`],
                                    'is-valid': !v$.latitude.$invalid && !errors[`latitude`]
                                }">

                            <div class="invalid-feedback">
                                <span v-if="v$.latitude.required.$invalid">{{ $t('validation.fieldRequired') }}<br /></span>
                            </div>
                            <template v-if="errors[`latitude`]">
                                <error-message v-for="(errorMessage, index) in errors[`latitude`]" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
                        ...
                        <div class="col-md-6">
                            <label class="form-label">{{ $t('global.longitude') }}</label>
                            <input type="number" class="form-control" v-model="v$.longitude.$model"
                                :placeholder="$t('global.longitude')" :class="{
                                    'is-invalid': v$.longitude.$error || errors[`longitude`],
                                    'is-valid': !v$.longitude.$invalid && !errors[`longitude`]
                                }">

                            <div class="invalid-feedback">
                                <span v-if="v$.longitude.required.$invalid">{{ $t('validation.fieldRequired') }}<br /></span>
                            </div>
                            <template v-if="errors[`longitude`]">
                                <error-message v-for="(errorMessage, index) in errors[`longitude`]" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
                        ...
                        <div class="col-md-6 mt-3">
                            <label for="price" class="form-label">{{$t('global.price')}}</label>
                            <input type="number" class="form-control" step="any" id="price" :placeholder="$t('global.price')"
                                   v-model.trim="v$.price.$model"
                                   :class="{'is-invalid': v$.price.$error ||errors[`price`],
                                'is-valid':!v$.price.$invalid && !errors[`price`]}">
                            <div class="invalid-feedback">
                                <span v-if="v$.price.required.$invalid">{{ $t('validation.fieldRequired') }}<br /> </span>
                                <span v-if="v$.price.numeric.$invalid">{{$t('validation.ThisFieldIsNumeric')}} <br /></span>
                            </div>
                            <template v-if="errors['price']">
                                <error-message v-for="(errorMessage, index) in errors['price']" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
...
export default {
    ...
    setup(props){
       ...
       function defaultData(){
           ...
           submitdata.data.latitude = '';
           submitdata.data.longitude = '';
           submitdata.data.price = '';
            ...
            setTimeout(async () => {
                ...
                adminApi.get(`dashboard/areas/${id.value}`)
                    .then((res) => {
                        ...
                        submitdata.data.latitude = l.latitude;
                        submitdata.data.longitude = l.longitude;
                        submitdata.data.price = l.price;
