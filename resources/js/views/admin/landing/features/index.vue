            ...
            <div class="col-xl-12">
                ...
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        <h6 class="card-title mb-0">قائمة المميزات</h6>
                        <button v-if="permission.includes('landing feature create')"
                            @click="showModelCreate" class="btn btn-sm btn-primary-light"
                            data-bs-toggle="modal" data-bs-target="#feature-modal">
                    ...
                    <div class="card-body">
                        <div class="table-responsive mb-2">
                            <table class="table text-nowrap table-striped">
                                <thead><tr>
                                    <th>#</th><th>الأيقونة</th><th>العنوان (EN)</th><th>العنوان (AR)</th>
                                    <th>الترتيب</th><th>الحالة</th><th>إجراء</th>
                                </tr></thead>
                                <tbody v-if="data && data.length">
                                    <tr v-for="(item, index) in data" :key="item.id">
                                        <td>{{ index + 1 }}</td>
                                        <td><i :class="'fas ' + item.icon" style="font-size:18px;color:#2563EB;"></i><small class="ms-1 text-muted">{{ item.icon }}</small></td>
                                        <td>{{ item.title_en }}</td>
                                        <td>{{ item.title_ar }}</td>
                                        ...
                                        <td>
                                            <div class="hstack gap-2">
                                                ...
                                                <button v-if="permission.includes('landing feature delete')"
...
export default {
    ...
    setup() {
        ...
        onBeforeMount(() => { uri.value = 'landing/features'; getData(); });
