<template>
    <div class="modal fade" id="problem-item-modal" tabindex="-1" aria-hidden="true">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" v-model="form.title_ar" maxlength="100"
                                :class="{'is-invalid': errors.title_ar}" placeholder="ضياع بيانات العملاء">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" v-model="form.title_en" maxlength="100"
                                :class="{'is-invalid': errors.title_en}" placeholder="Lost Customer Data">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">شارة التأثير (عربي) <small class="text-muted">(اختياري)</small></label>
                            <input type="text" class="form-control" v-model="form.impact_label_ar" maxlength="100"
                                :class="{'is-invalid': errors.impact_label_ar}" placeholder="تأثير عالي">
                            <div class="invalid-feedback" v-if="errors.impact_label_ar">{{ errors.impact_label_ar[0] }}</div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">شارة التأثير (إنجليزي) <small class="text-muted">(اختياري)</small></label>
                            <input type="text" class="form-control" v-model="form.impact_label_en" maxlength="100"
                                :class="{'is-invalid': errors.impact_label_en}" placeholder="High Impact">
                            <div class="invalid-feedback" v-if="errors.impact_label_en">{{ errors.impact_label_en[0] }}</div>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-12">
                            <label class="form-label fw-semibold">الأيقونة / الصورة <small class="text-muted">(اختياري — placeholder افتراضي)</small></label>
                        ...
                        <div class="col-12 d-flex align-items-center">
                            <div class="custom-toggle-switch d-flex align-items-center">
                                <input id="problem-item-active" type="checkbox" v-model="form.is_active">
                                <label for="problem-item-active" class="label-primary"></label>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const defaultForm = () => ({
            ...
            impact_label_ar: '', impact_label_en: '',
            sort_order: 1, is_active: true, current_image: null,
        });
        const form = ref(defaultForm());

        const modalTitle = computed(() => {
            if (props.type === 'create') return 'إضافة مشكلة جديدة';
            const title = props.dataRow?.title_ar || props.dataRow?.title_en || '';
            return title ? `تعديل المشكلة # ${title}` : 'تعديل المشكلة';
        });
        ...
        const loadForm = () => {
            ...
            if (row && type === 'edit') {
                form.value = {
                    ...
                    impact_label_ar: row.impact_label_ar || '',
                    impact_label_en: row.impact_label_en || '',
        ...
        onMounted(() => {
            const el = document.getElementById('problem-item-modal');
            if (el) el.addEventListener('hidden.bs.modal', () => emit('closed'));
        });

        onBeforeUnmount(() => {
            const el = document.getElementById('problem-item-modal');
            if (el) el.removeEventListener('hidden.bs.modal', () => emit('closed'));
        });
        ...
        const submit = () => {
            ...
            fd.append('impact_label_ar', form.value.impact_label_ar || '');
            fd.append('impact_label_en', form.value.impact_label_en || '');
            ...
            const url = props.type === 'edit'
                ? `dashboard/landing/problem-items/${props.dataRow.id}`
                : 'dashboard/landing/problem-items';
