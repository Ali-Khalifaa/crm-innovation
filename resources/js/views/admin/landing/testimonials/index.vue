                                ...
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control" v-model="form.subtitle.ar" maxlength="100"
                                            :class="{'is-invalid': fieldError('subtitle.ar')}" placeholder="آراء عملائنا">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control" v-model="form.subtitle.en" maxlength="100"
                                            :class="{'is-invalid': fieldError('subtitle.en')}" placeholder="Client Testimonials">
                                    ...
                                    <div class="col-12">
                                        <div class="custom-toggle-switch d-flex align-items-center">
                                            <input id="testimonial-section-active" type="checkbox" v-model="form.is_active">
                                            <label for="testimonial-section-active" class="label-primary"></label>
                ...
                <div class="col-12">
                    <div class="card custom-card">
                        <div class="card-header justify-content-between">
                            <h6 class="card-title mb-0"><i class="bx bx-message-square-detail me-2"></i>آراء العملاء</h6>
                        ...
                        <div class="card-body">
                            <div class="table-responsive mb-2">
                                <table class="table text-nowrap table-striped">
                                    <thead><tr>
                                        <th>#</th><th>الصورة</th><th>الاسم (AR)</th><th>الاسم (EN)</th>
                                        <th>التقييم</th><th>الترتيب</th><th>الحالة</th><th>إجراء</th>
                                    </tr></thead>
                                    <tbody v-if="items.length">
                                        <tr v-for="(item, index) in items" :key="item.id">
                                            <td>{{ index + 1 }}</td>
                                            <td>
                                                <img v-if="itemImageUrl(item)" :src="itemImageUrl(item)" style="height:40px;width:40px;border-radius:50%;object-fit:cover;" alt="">
                                            ...
                                            <td>
                                                <span v-for="s in 5" :key="s" class="text-warning" style="font-size:11px;">
                                                    <i :class="s <= item.rating ? 'ri-star-fill' : 'ri-star-line'"></i>
                                                </span>
                                            </td>
...
export default {
    ...
    setup() {
        ...
        const loadSection = () => adminApi.get('dashboard/landing/testimonials').then(res => {
        ...
        const loadItems = () => adminApi.get('dashboard/landing/testimonial-items', { params: { per_page: 100 } }).then(res => {
        ...
        const submitSection = () => {
            ...
            adminApi.post('dashboard/landing/testimonials', fd)
                .then(() => {
        ...
        const deleteItem = (id, index) => {
            Swal.fire({
                title: 'هل أنت متأكد؟',
                text: 'سيتم حذف هذا الرأي نهائياً',
                icon: 'warning',
                ...
                adminApi.delete(`dashboard/landing/testimonial-items/${id}`).then(() => {
