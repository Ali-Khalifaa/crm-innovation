            ...
            <div class="col-xl-12">
                ...
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        <h6 class="card-title mb-0">قائمة الأسئلة الشائعة</h6>
                        <button v-if="permission.includes('landing faq create')"
                            @click="showModelCreate" class="btn btn-sm btn-primary-light"
                            data-bs-toggle="modal" data-bs-target="#faq-modal">
                                ...
                                <tbody v-if="data && data.length">
                                    <tr v-for="(item, index) in data" :key="item.id">
                                        ...
                                        <td style="max-width:280px;white-space:normal;">
                                            <span class="d-block text-truncate" style="max-width:280px;" :title="item.question_en">{{ item.question_en }}</span>
                                        </td>
                                        <td style="max-width:280px;white-space:normal;">
                                            <span class="d-block text-truncate" style="max-width:280px;" :title="item.question_ar">{{ item.question_ar }}</span>
                                        </td>
                                        ...
                                        <td>
                                            <div class="hstack gap-2">
                                                ...
                                                <button v-if="permission.includes('landing faq delete')"
...
export default {
    ...
    setup() {
        ...
        search.value = { searchKey: '', searchInTranslations: false, columns: ['question_en'] };
        onBeforeMount(() => { uri.value = 'landing/faqs'; getData(); });
