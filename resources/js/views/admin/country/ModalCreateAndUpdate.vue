                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-12 mt-3">
                            <label for="domain" class="form-label">{{$t('global.domain')}}</label>
                            <input type="text" class="form-control" id="domain" :placeholder="$t('global.domain')"
                                v-model.trim="v$.domain.$model"
                                :class="{'is-invalid': v$.domain.$error ||errors[`domain`],
                                'is-valid':!v$.domain.$invalid && !errors[`domain`]}">
                            <div class="invalid-feedback">
                                <span v-if="v$.domain.required.$invalid">{{ $t('validation.fieldRequired') }}<br /> </span>
                            </div>
                            <template v-if="errors['domain']">
                                <error-message v-for="(errorMessage, index) in errors['domain']" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
            ...
            setTimeout(async () => {
                ...
                adminApi.get(`dashboard/countries/${id.value}`)
                    .then((res) => {
                        ...
                        submitdata.data.domain = l.domain;
